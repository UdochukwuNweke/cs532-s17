...
def getFriendCountDict(friendsTxt):
		...
		if( graph is not None ):
			...
			for node in nodes:
				try:
					...
					friendsDict[ node['id'] ] = {'friend_name': friendName,'friend_count': int(friendCount)}
				except:
					errorMsg()
	...
	try:
		...
		for friendID in sortedIDs:
			print(friendsDict[friendID]['friend_name'], friendsDict[friendID]['friend_count'])
			outfile.write( friendsDict[friendID]['friend_name'] + ', ' + str(friendsDict[friendID]['friend_count']) + '\n' )
