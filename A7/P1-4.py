...
def getUserDict(path='./data/movielens'):
	...
	for line in open(path + '/u.user'):
		...
		users[userid] = {'userid': userid, 'age': int(age), 'gender': gender, 'transformed-gender': transformedGender, 'occupation': occupation, 'transformed-occupation': transformedOccupation, 'zipcode': zipcode}
...
def getClosestTriple(users):
	...
	for userid in users:
		...
		users[userid]['distance'] = euclideanDistance([age, gender, occupation], [32, 0, 5])
...
def question1(users, prefs):
	...
	##udoDict = {'transformed-occupation': 5, 'age': 32, 'transformed-gender': 0, 'zipcode': '23508', 'gender': 'F', 'occupation': 'student'}
	for user in closestTriple:
		print('user:', user)
		userRatedMovies = prefs[ user['userid'] ]
		
		topRatedMovies = sorted(userRatedMovies, key=lambda moviename:userRatedMovies[moviename])
...
def question3(userid, prefs):
	...
	print('top 5 recommendations for films that the substitute ' + userid + ' should see')
	ratings = getRecommendations(prefs, userid)
	for i in range(len(ratings)-1, len(ratings)-6, -1):
		print('\t', ratings[i] )

	print('the bottom 5 recommendations that the substitute ' + userid + ' should not see')
	for i in range(0, 5):
		print('\t', ratings[i] )
