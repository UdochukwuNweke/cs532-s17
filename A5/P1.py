...
def GirvinNewmanAlg(graph, maxClusterCount):
	...
	vertex_colors = {0: 'black', 1: 'light blue', 2: 'red', 3: 'green', 4: 'pink', 5: 'brown'}
	...
	while graph.ecount() > 0 and clusterCount < maxClusterCount:
		...
		style = {}
		style['vertex_color'] = [vertex_colors[node['vertex_color']] for node in graph.vs]
		...
		drawGraph(graph, vertex_colors, str(iterationCounter)+ '-' + str(clusterCount) + '.pdf' )
...
drawGraph(karateClub, {1.0: 'light blue', 2.0: 'red'}, 'karateClub.postsplit.pdf')
