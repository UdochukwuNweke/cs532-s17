...
def getLink(source_screen_name, source_name, target_screen_name, target_name, outfile):
	link = api.show_friendship(source_screen_name=source_screen_name, target_screen_name=target_screen_name)
	
	if( link[0].following ):
		print('\t\t', source_name, 'follows', target_name)
		#arrow from source to target
		#graphEdges.append( {'source': source_name, 'target': target_name} )
		outfile.write('{"source": "' + source_name + '", "target": "' + target_name + '"},\n')
	...
	if( link[0].followed_by ):
		...
		outfile.write('{"source": "' + target_name + '", "target": "' + source_name + '"},\n')
...
def writeFirstDegreeLinks(outfile):
	...
	for follower in followersList:
		outfile.write('{"source": "Michael L. Nelson", "target": "' + follower['name'] + '"},\n')
...
def computeFriendshipLink(outfile):
	...
	for i in range(0, 100):
		for j in range(i+1, len(followersList)):
			...
			getLink(source_screen_name=source_screen_name, source_name=source_name, target_screen_name=target_screen_name, target_name=target_name, outfile=outfile)
...
def getFriendsDict(twitterName):
	...
	try:
		rateLimitQuota = api.rate_limit_status()['resources']['followers']['/followers/list']['remaining']
