#modified from: https://github.com/arthur-e/Programming-Collective-Intelligence/tree/master/chapter6
import feedparser
import re
import os, sys


def getClassData(feed, classifier, inputFilename, mode, maxItemsToTestOrTrain, wordOrEntry='word'):

  if( len(feed) > 0 and len(inputFilename) > 0 and (mode == 'train' or mode == 'test') and maxItemsToTestOrTrain > 0 and (wordOrEntry == 'word' or wordOrEntry == 'entry')):
        ...
        if( title.lower() == entry['title'].strip().lower() ):
          ...
          if( mode == 'train' ):
            ...
            try:
              ...
              print( title + ' <> ' + str(classPredictionProbability) + ' <> ' + prediction + ' <> ' + actualClassLabel )
              outputFile.write(title + ' <> ' + str(classPredictionProbability) + ' <> ' + prediction + ' <> ' + actualClassLabel + '\n')
            except:
              exc_type, exc_obj, exc_tb = sys.exc_info()
              fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
              print((fname, exc_tb.tb_lineno, sys.exc_info() ))
